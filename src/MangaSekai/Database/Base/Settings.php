...
abstract class Settings implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\MangaSekai\\Database\\Map\\SettingsTableMap';
    ...
    /**
     * Initializes internal state of MangaSekai\Database\Base\Settings object.
     */
     ...
     * @return $this|\MangaSekai\Database\Settings The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[SettingsTableMap::COL_ID] = true;
        }
    ...
    public function setName($v)
    {
        ...
        if ($this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[SettingsTableMap::COL_NAME] = true;
        }
    ...
    public function setValue($v)
    {
        ...
        if ($this->value !== $v) {
            $this->value = $v;
            $this->modifiedColumns[SettingsTableMap::COL_VALUE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : SettingsTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : SettingsTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            $this->name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : SettingsTableMap::translateFieldName('Value', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = SettingsTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\MangaSekai\\Database\\Settings'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(SettingsTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(SettingsTableMap::COL_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'name';
        }
        if ($this->isColumnModified(SettingsTableMap::COL_VALUE)) {
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\MangaSekai\Database\Settings
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\MangaSekai\Database\Settings The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(SettingsTableMap::COL_ID)) {
            $criteria->add(SettingsTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(SettingsTableMap::COL_NAME)) {
            $criteria->add(SettingsTableMap::COL_NAME, $this->name);
        }
        if ($this->isColumnModified(SettingsTableMap::COL_VALUE)) {
            $criteria->add(SettingsTableMap::COL_VALUE, $this->value);
        }
    ...
    {
        $criteria = ChildSettingsQuery::create();
        $criteria->add(SettingsTableMap::COL_ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \MangaSekai\Database\Settings (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \MangaSekai\Database\Settings Clone of current object.
     * @throws PropelException
     */
